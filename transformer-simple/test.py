import torch
import numpy as np
from parser import args
from torch.autograd import Variable
from utils import subsequent_mask
import nltk.translate.bleu_score as bleu
from nltk.translate.bleu_score import SmoothingFunction
import time
import jieba


def log(data, timestamp):
    file = open(f'log/log-{timestamp}.txt', 'a', encoding='utf-8')
    file.write(data)
    file.write('\n')
    file.close()

def greedy_decode(model, src, src_mask, max_len, start_symbol):
    # 这个函数使用了贪心策略来生成输出序列，即在每个时间选择概率最大的输出符号
    # ys即为输出序列

    # 调用编码器，将源序列和源序列掩码编码成一个向量序列，作为decoder的输入
    memory = model.encode(src, src_mask)
    # 初始化输出序列ys为一个包含单个元素的一维张量
    ys = torch.ones(1, 1).fill_(start_symbol).type_as(src.data)
    for i in range(max_len-1):
        out = model.decode(memory, src_mask,
                           Variable(ys),
                           Variable(subsequent_mask(ys.size(1))
                                    .type_as(src.data)))
        prob = model.generator(out[:, -1])
        _, next_word = torch.max(prob, dim = 1)  # 获得下一个输出符号。
        next_word = next_word.data[0]
        ys = torch.cat([ys,
                        torch.ones(1, 1).type_as(src.data).fill_(next_word)], dim=1)  # 将输出符号添加到输出序列的末尾
    return ys


def test_bleu(data, model):
    smoothie = SmoothingFunction().method0
    total_bleu = 0
    count = 0
    timestamp = time.time()

    with torch.no_grad():
        for i in range(len(data.dev_en)):
            en_sent = " ".join([data.en_index_dict[w] for w in data.dev_en[i]])
            cn_sent = " ".join([data.cn_index_dict[w] for w in data.dev_cn[i]])

            src = torch.from_numpy(np.array(data.dev_en[i])).long().to(args.device)
            src = src.unsqueeze(0)
            src_mask = (src != 0).unsqueeze(-2)

            out = greedy_decode(model, src, src_mask, max_len=args.max_length, start_symbol=data.cn_word_dict["BOS"])

            translation = []
            for j in range(1, out.size(1)):
                sym = data.cn_index_dict[out[0, j].item()]
                if sym != 'EOS':
                    translation.append(sym)
                else:
                    break

            reference = [list(jieba.cut(cn_sent))]  # 使用jieba分词器分词
            candidate = list(jieba.cut(" ".join(translation)))  # 使用jieba分词器分词

            bleu_score = bleu.sentence_bleu(reference, candidate, smoothing_function=smoothie)
            total_bleu += bleu_score
            count += 1

            print("English: ", en_sent)
            print("Chinese: ", cn_sent)
            print("Translation: ", " ".join(translation))
            print("Current BLEU:", bleu_score)

            log(en_sent, timestamp)  # 将句子写入日志
            log(cn_sent, timestamp)
            log("Translation: " + " ".join(translation) + "\n", timestamp)

    average_bleu = total_bleu / count if count > 0 else 0
    return average_bleu
