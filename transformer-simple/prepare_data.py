...
class PrepareData:
    def __init__(self):
        ...
        self.test_en, self.test_cn = self.load_data(args.test_file)
        ...
        # id化
        # id化通常指的是将文本或字符转换成唯一的整数id，即建立一个映射
        # 这种方法可以简化计算
        ...
        self.test_data = self.splitBatch(self.test_en, self.test_cn, args.batch_size)
...
class Batch:
    ...
    def __init__(self, src, trg=None, pad=0):
        ...
        self.src = src
        # src != pad生成bool数组，且数组所有元素均为TRUE
        # 此处unsqueeze扩展维度，表示在倒数第二个位置扩展维度
        self.src_mask = (src != pad).unsqueeze(-2)
        if trg is not None:
            self.trg = trg[:, :-1]  # 截掉最后EOS
            self.trg_y = trg[:, 1:]  # 截掉开头BOS
            self.trg_mask = \
                self.make_std_mask(self.trg, pad)  # 对target掩蔽
    ...
    def make_std_mask(tgt, pad):
        ...
        # 调用了utils的subsequent_mask方法得到上三角布尔矩阵
        # Variable()封装Tensor，并储存tensor的梯度，与tgt_mask做运算
